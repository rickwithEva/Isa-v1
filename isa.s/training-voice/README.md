...
1. `virtualenv training-voice.venv`
2. `source training-voice.venv/bin/activate`
...
`docker build -f cpu.Dockerfile -t training-voice .`

If you are running on a cuda enabled machine 
`docker build -f Dockerfile -t training-voice .`
...
2. Train model
    1. use `train.py` to fine tune. checkout the [train.py](https://github.com/LearnedVector/A-Hackers-AI-Voice-Assistant/blob/master/training-voice/speechrecognition/neuralnet/train.py#L115) argparse for other arguments
    ```
